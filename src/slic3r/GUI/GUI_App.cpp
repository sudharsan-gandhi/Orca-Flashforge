...
void GUI_App::check_new_version_sf(bool by_user, bool use_uid)
{
    ...
    wxString version_url_check = format("%s?app_id=%d&platform=%d&version=v%s", VERSION_URL_CHECK, APP_ID, PLATFORM_ID, Orca_Flashforge_VERSION);
    if (use_uid) {
...
bool GUI_App::load_language(wxString language, bool initial)
{
    ...
    if (!wxLocale::IsAvailable(locale_language_info->Language)) {
	    ...
	    wxString message = "Switching Orca-Flashforge to language " + requested_language_code + " failed.";
