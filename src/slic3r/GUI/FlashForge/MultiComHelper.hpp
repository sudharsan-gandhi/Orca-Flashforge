...
public:
    ...
    int64_t getMonitorMessage(std::string& data, const std::string& language, int msTimeout);

    int64_t retrySliceTask(const std::string& id, int msTimeout);

    int64_t cancelSliceTask(const std::string& id, int msTimeout);
