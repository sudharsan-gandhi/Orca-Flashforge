...
void SingleDeviceState::fillValue(const com_dev_data_t& data,bool wanDev)
{
    ...
    if (!isSupportCamera) {
        ...
        if (stram_url.empty() && data.connectMode == COM_CONNECT_LAN &&
            ...
            // Some firmware reports camera support without also reporting a
            // stream URL on a LAN connection. The camera serves the usual MJPEG
            // endpoint regardless, so fall back to the local address rather than
            // leaving the panel with nothing to load.
