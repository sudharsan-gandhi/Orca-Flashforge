...
#ifdef _WIN32
        int side_tools_height = sizeAll.y /*- rect.y*/ - toolPosy - m_side_tools->GetRect().height - m_side_tools->getConnectInfoHeight() -
                                10;
#else
        int side_tools_height = sizeAll.y /*- rect.y*/ - toolPosy - m_side_tools->GetRect().height - m_side_tools->getConnectInfoHeight() -
                                40;
...
void MonitorPanel::show_status(int status)
{
    ...
    if ((status & (int) MonitorStatus::MONITOR_NO_PRINTER) != 0) {
        ...
        if (((status & (int) MonitorStatus::MONITOR_DISCONNECTED) != 0) ||
            ((status & (int) MonitorStatus::MONITOR_DISCONNECTED_SERVER) != 0) ||
            ((status & (int) MonitorStatus::MONITOR_CONNECTING) != 0) || ((status & (int) MonitorStatus::MONITOR_CONNECTED_FAILED) != 0)) {
            set_default();
        }
        m_tabpanel->Layout();
    }
