                                                               ...
                                                               int                              preserve_outer_count)
{
    ...
    auto clip_paths = [&](const ExtrusionPaths &src_paths, ExtrusionPaths &dst_paths) {
        ...
        for (const ExtrusionPath &p : src_paths) {
            // Ported: ExtrusionPath::polyline is Polyline3 in this base (Z-contouring).
            // Clipping is planar, so project to 2D, clip, then lift back. The source Z is
            // preserved so a constant non-zero Z survives the round trip; a varying-Z
            // contoured path would be flattened to its first Z, which is acceptable
            // because wave overhangs and Z-contouring are not supported on one region.
            const coord_t src_z = p.polyline.points.empty() ? coord_t(0) : p.polyline.points.front().z();
            ...
            for (Polyline &pl : kept) {
                ...
                if (pl.points.size() < 2)
                    continue;
                ExtrusionPath np(p);
                np.polyline = Polyline3(pl, src_z);
    ...
    for (const ExtrusionEntity *ent : src.entities) {
        ...
        if (const ExtrusionLoop *loop = dynamic_cast<const ExtrusionLoop*>(ent)) {
            ...
            // Ported: same 2D projection / lift as clip_paths above.
            const coord_t src_z = path->polyline.points.empty() ? coord_t(0) : path->polyline.points.front().z();
